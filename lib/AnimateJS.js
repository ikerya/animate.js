...
function AnimateJS(el, data) {
    ...
    const keyframes = data.keyframes.reduce((keyframes, { from, to, duration = delay, timing, ...rest }, index) => {
        ...
        const data = {};
        const props = Object.entries(rest).map(([ prop, value ]) => {
            const from = value[0].toString().split(',').map((f) => +f);
            const to = (value[1] || 0).toString().split(',').map((t) => +t);
            const customType = value[2];
            const isInitialValue = typeof value[1] === 'undefined';
            const act = customType || ((from > to) ?
                'dec':
                'inc');
            ...
            function onTick(progress) {
                const change = isInitialValue ?
                    ...
                    if (act === 'dec') {
                    lastValue = from - change;
                    lastValue = from.map((f, i) => f - change[i]);
                } else {
                    lastValue = from + change;
                    lastValue = from.map((f, i) => f + change[i]);
                }
        ...
        function start(cb) {
            ...
            console.log('start', id, { realDuration, steps });
